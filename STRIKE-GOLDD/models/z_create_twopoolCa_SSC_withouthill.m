...
% Hill coefficients not included, used as constants 2, 2, 4 for n, m, p
syms beta mu0 mu1 vm2 vm3 k2 kr ka k kf
p = [beta; mu0; mu1; vm2; vm3; k2; kr; ka; k; kf;];
...
% v3 = ((vm3 * (Z ^ p)) / (ka ^ p + (Z) ^ p)) * ((Y ^ m_sup) / (kr ^ m_sup + Y ^ m_sup))
v3 = ((vm3 * (Z^4)) / (ka^4 + (Z)^4)) * ((Y^2) / (kr^2 + Y^2));
...
save('twopoolCa_SSC_withouthill','x','p','h','u','w','f','known_ics');